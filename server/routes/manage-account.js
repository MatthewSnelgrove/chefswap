...
router.get("/", async (req, res) => {
  ...
  const account = camelize(
    await pool.query(
      `SELECT account.username, account.email, account.bio,
    ...
    (circle_uid) WHERE account_uid=$1`,
      [accountUid]
    )
  ).rows[0];
  ...
  const images = camelize(
    await pool.query(
      `SELECT image_link, timestamp FROM image WHERE 
    account_uid=$1`,
      [accountUid]
    )
  ).rows;
...
router.post("/username", async (req, res) => {
  ...
  await pool.query(`UPDATE account SET username=$1 WHERE account_uid=$2`, [
    username,
    accountUid,
  ]);
...
router.post("/email", async (req, res) => {
  ...
  await pool.query(`UPDATE account SET email=$1 WHERE account_uid=$2`, [
    email,
    accountUid,
  ]);
...
router.post("/address", checkAuth, async (req, res) => {
  ...
  const addressUid = camelize(
    await pool.query(`SELECT address_uid FROM account WHERE account_uid=$1`, [
      accountUid,
    ])
  ).rows[0].addressUid;
  ...
  await pool.query(`UPDATE address SET ${setString} WHERE address_uid=$1`, [
    addressUid,
  ]);
...
router.post("/bio", checkAuth, async (req, res) => {
  ...
  await pool.query(`UPDATE account SET bio=$1 WHERE account_uid=$2`, [
    bio,
    accountUid,
  ]);
...
router.delete("/gallery/", checkAuth, async (req, res) => {
  ...
  const image = (
    await pool.query(
      `DELETE FROM image WHERE image_link=$1 RETURNING image_link`,
      [imageName]
    )
  ).rows[0];
  if (!image) {
...
router.post(
  ...
  async (req, res) => {
    ...
    blobStream.on("finish", async () => {
      await pool.query(
        `INSERT INTO image (account_uid, image_link) VALUES ($1, $2)`,
