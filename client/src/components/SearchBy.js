import { React, useRef } from "react"
import "./styles/SearchBy.css"

function SearchBy(props) {
    const searchRef = useRef()
    
    return (
        <div style={props.containerStyle ? props.containerStyle: {}}>
            <span style={props.textStyle ? props.textStyle: {}}>Search By:</span>
            <div className="input-container">
                <input ref={searchRef} style={props.inputStyle ? props.inputStyle: {}} pattern="[^!*'();:@&=+$,/?%#\\]" className="username-search" 
                onKeyUp={(e) => {
                    if (e.key != "Enter") {return}
                    goToPage(searchRef)
                }}
                ></input>
                <button className="search-button" onClick={(e) => goToPage(searchRef)}>
                    <span class="material-symbols-outlined search-tag">search</span>
                </button>
            </div>

      </div>
    )   
}

function goToPage(searchRef) {
    window.location = `${global.config.pages.searchSwaps}/${searchRef.current.value}`
}

export default SearchBy