...
function FindUserPage(props) {
    ...
    const [users, setUsers] = useUsernameSearch(userAddress, usernameQuery, userObserver)
    ...
    return (
        <OnlyLoggedIn>
            <div className="navbar-margin container search-user-container">
                <SearchBy containerStyle={{marginBottom: "10px", display: "flex", flexWrap: "wrap"}} textStyle={{fontSize: "30px", marginRight: "10px"}} inputStyle={{fontSize: "30px", display: "flex"}} />
                        ...
                        return <SwapListing 
                            ...
                            finalColJsx={<>
                                <button style={{border: "0px"}} onClick={(e) => {
                                    createNewSwapRequest(user, profile.accountUid, profile.username, setUsers)
                                }}>
                                    <span style={{fontSize: "60px"}} class="material-symbols-outlined small-swap-button">swap_horiz</span>
                                </button>
...
function createNewSwapRequest(user, requesteeUid, requesteeUsername, setUsers) {
    // newSwapRequest(user.accountUid, requesteeUid, requesteeUsername).then(() => {
      setUsers((curUsers) => curUsers.filter((otherUser) => otherUser.profile.accountUid != requesteeUid))
    // })
  }
