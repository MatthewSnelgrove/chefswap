  ...
  return (
    <>
      <SwapListBlueprint type={"Ongoing"} data={props.swapListOngoing}>
        {props.swapListOngoing.map((person, index) => (
          ...
          <SwapListing
            ...
            finalColJsx={
              <EndSwap person={person} setCurPerson={setCurPerson} />
            }
          />
        ))}
      </SwapListBlueprint>
      <ConfirmRatingModal
        ...
        setSwapListPast={props.setSwapListPast}
      />
    </>
  );
}
...
function EndSwap(props) {
  ...
  return (
    <div className={styles.button_wrapper}>
      <button
        className={styles.end_swap_button}
        title="End swap with user"
...
function endSwap(user, curPerson, setSwapListOngoing, setSwapListPast) {
  changeSwapStatus(
    user.accountUid,
    curPerson.accountUid,
    curPerson.username,
    curPerson.requestTimestamp,
    "ended"
  ).then(() => {
    setSwapListOngoing((curSwapListOngoing) =>
      curSwapListOngoing.filter(
        (userFilter) => userFilter.username != curPerson.username
      )
    );
            ...
            <button
              ...
              onClick={(e) =>
                endSwap(
                  user,
                  curPerson,
                  props.setSwapListOngoing,
                  props.setSwapListPast
                )
              }
            >
              Confirm end swap
